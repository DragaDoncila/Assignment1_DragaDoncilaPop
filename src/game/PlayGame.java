...
/**
 * Class handles input/output for playing a game of Mineral Supertrumps in command line Created by
 * Draga on 6/09/2016.
 */
public class PlayGame {
  ...
  private static final String MAIN_MENU =
      "Main Menu\n" + "(P)lay game\n" + "(I)nstructions\n" + "(Q)uit\n" + "   >>> ";
  private static final String TURN_MENU =
      "Turn Menu\n" + "(V)iew hand details\n" + "(S)elect card\n" + "(C)ombo\n" + "(P)ass";

  private static final String[] validTrumpStrings = {
    "Cleavage", "Crustal Abundance", "Economic Value", "Hardness", "Specific Gravity"
  };
  private static final int BACK_VALUE = -1;
            ...
            if (gameWinners.contains(playerUp)) {
              ...
              if (superTrumpsGame.userIsUp()) {
                ...
                while (playerUp.isUser()) {
                  ...
                  if (!playerUp.isOut()) {
                    if (!isNewRound) {
                      playerUp.setPlayableCards(
                          superTrumpsGame.getLastPlayedCard(),
                          superTrumpsGame.getCurrentCategory());
                    }
                    if (playerUp.hasPlayableCards() || isNewRound) {
                      ...
                      switch (userChoice) {
                        ...
                        case "P":
                          ...
                          System.out.println(
                              playerUp.getName() + " passed. They're out for the round.");
                          waitForUser();
                          break;
                        case "C":
                          if (playerUp.hasCombo()) {
                            if (!superTrumpsGame.isFirstTurn()) {
                              ...
                              System.out.println(
                                  "COMBO PLAYED! " + playerUp.getName() + " plays again!");
                              isNewRound = checkForNewRound(superTrumpsGame);
                            } else {
                ...
                if (!playerUp.isOut()) {
                  if (superTrumpsGame.isFirstTurn()) {
                    ...
                    playerUp.setPlayableCards(
                        superTrumpsGame.getLastPlayedCard(), superTrumpsGame.getCurrentCategory());
                    if (playerUp.hasPlayableCards()) {
                      superTrumpsGame.playTurn();
                      if (superTrumpsGame.comboWasPlayed()) {
                        System.out.println(
                            "COMBO PLAYED! " + playerUp.getName() + " is going again.");
                      } else {
                        displayTurnResults(superTrumpsGame, playerUp);
                      }
              ...
              if (superTrumpsGame.hasWon(lastToPlay) && lastToPlay.getName().equals(playerUp.getName())) {
                System.out.println(
  ...
  /**
   * Gets a number from the user in the range of allowed number of players and doesn't return it
   * until it's valid
   *
  ...
  private static int getValidNumPlayers() {
    ...
    int numPlayers = 0;
    System.out.println(
        "This is a game for " + Game.MIN_PLAYERS + " to " + Game.MAX_PLAYERS + " people.");
  ...
  /**
   * Establishes and prepares a Mineral Supertrumps game with a number of players, a dealer and
   * dealt hands.
   *
  ...
  /**
   * Gets a trump choice from the user and only returns the associated string if the user makes a
   * valid choice
   *
  ...
  /**
   * Get a valid card choice from the user that is playable on the last played card, get a trump
   * category if required, and play the turn
   *
   * @param superTrumpsGame the current game
   * @param playerUp the current player
   */
  ...
  static void displayTurnResults(Game superTrumpsGame, Player playerUp) {
    ...
    waitForUser();
    System.out.println(
        playerUp.getName() + " played " + superTrumpsGame.getLastPlayedCard().getTitle());
    System.out.println("Current trump category is: " + superTrumpsGame.getCurrentCategory());
    System.out.println(
        "Current trump value is: "
            + superTrumpsGame.getLastPlayedCard().getTrumpVal(superTrumpsGame.getCurrentCategory())
            + "\n");
  }
  ...
  private static void checkForSuperTrump(Game superTrumpsGame) {
    if (superTrumpsGame.getLastPlayedCard() != null
        && superTrumpsGame.getLastPlayedCard().isTrump()) {
      superTrumpsGame.setAllPlayersIn();
      superTrumpsGame.resetNumPasses();
  ...
  private static void viewCardDetails(Player playerUp, Game superTrumpsGame) {
    ...
    if (cardChoice != BACK_VALUE) {
      ...
      if (superTrumpsGame.getLastPlayedCard() != null) {
        System.out.println(
            String.format("%-30s", "Last Played Value: ")
                + superTrumpsGame.getLastPlayedCard().getTrumpVal(currentCategory));
      }
      System.out.println("------------------------------------------");
    }
  }
    ...
    while (!validChoice) {
      try {
        ...
        if (userChoice.equals("B")) {
          ...
          System.out.println(
              "That is not a valid choice. Please enter valid number or B to return to turn menu.");
          System.out.printf("Your choice >>> ");
          userChoice = input.nextLine().toUpperCase();
        }
      }
